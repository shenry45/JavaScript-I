...
console.log(`The last car in inventory is the ${lastCar["car_make"]} ${lastCar["car_model"]}.`);
...
for (let i = 0; i < inventory.length; i++) {
  const curCar = inventory[i]["car_model"];

  if (carModels.indexOf(curCar) === -1) {
    carModels.push(curCar);
  }
}
...
for (let i = 0; i < inventory.length; i++) {
  const curCar = inventory[i]["car_year"];

  if (carModels.indexOf(curCar) === -1) {
    carYears.push(curCar);
  }
}
...
for (let i = 0; i < inventory.length; i++) {
  ...
  if (oldCars.indexOf(curCar) === -1) {
    oldCars.push(curCar);
  }
}

console.log(`${oldCars.length} cars in inventory were made before the year 2000.`);
...
for (let i = 0; i < inventory.length; i++) {
  const curCar = inventory[i]["car_make"];

  if (curCar == "BMW" || curCar == "Audi") {
